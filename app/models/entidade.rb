class Entidade < ActiveRecord::Base
	has_attached_file :chave_privada

	FILES_NAME_PERMIT = [/jks\Z/]
	FILES_CONTENT_TYPE = ['application/jks']
	...
	validates :sigla, length: {maximum: 6, too_long: "Máximo de #{count} caracteres permitidos."},
	...
	validates :password, length:{maximum: 20, too_long: "Máximo de #{count} caracteres permitidos!"}, allow_blank: true
	...
	validates_attachment_size :chave_privada, :less_than => 1.megabytes
	validates_attachment_file_name :chave_privada, :matches => FILES_NAME_PERMIT
	validates_attachment_content_type :chave_privada, :content_type => FILES_CONTENT_TYPE
